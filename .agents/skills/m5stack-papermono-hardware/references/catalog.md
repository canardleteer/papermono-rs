...
| Firmware | Kind | Notes |
| --- | --- | --- |
| [M5PaperMono-UserDemo](https://github.com/m5stack/M5PaperMono-UserDemo) | Official ESP-IDF eval | MIT. Display, clock, battery, brightness, IMU, mic, RGB, buzzer, Wi-Fi, NFC, LoRa, TF, sleep. `partitions.csv` is intent |
...
SKU: burn Lite firmware on Lite. Do not assume a full-SKU
image is safe on Lite (NFC/LoRa init).

## Native ESP-IDF (no Arduino)

OTP-Demo is the panel path to read first. UserDemo is the HAL
path. This repository does not add an IDF project.
