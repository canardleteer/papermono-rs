...
| Soft mono status redraw | `C153-Lite` | 2026-09-04 embassy-debug. Same-card Legend / Bluetooth / Wi-Fi `paint_mono_fast(..., soft = true)` stays on OTP `Partial` past `PARTIALS_BEFORE_FULL` (6) so PIN / client / survey updates do not flash `MonoFull`. Card navigation still takes `MonoFull` when the budget is due. [display.md](display.md) |
