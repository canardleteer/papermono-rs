...
| `ci` | no | Host + firmware clippy: fmt, host clippy/test, `cargo +esp clippy -p simple-debug-fw`, `embassy-debug-fw` (default, `--no-default-features`, then `touch` / `mic` / `panel` / `touch,radio` / `touch,sleep` / `touch,panel,orient` with `--no-default-features`), rumdl, machete, audit. Needs esp toolchain |
