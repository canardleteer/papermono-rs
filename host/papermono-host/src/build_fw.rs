//! Host-only Xtensa build + `save-image`. No USB.
...
pub enum FirmwareImage {
    ...
    SimpleDebug,
    /// Embassy staged image (`embassy-debug-fw`).
    EmbassyDebug,
}
...
/// `cargo +esp build` profile, then host-only `save-image`.
#[derive(Debug, Clone)]
...
pub struct BuildFwOutput {
    /// Linked ELF (`simple-debug-fw`).
    pub elf: PathBuf,
    /// `espflash save-image` payload (not an ELF).
    pub bin: PathBuf,
}

/// Same hint as `ci`: source the script `espup` printed.
...
/// `-Zbuild-std=core,alloc`. Does not open USB.
pub fn build_fw(repo_root: &Path, args: &BuildFwArgs) -> Result<BuildFwOutput, Error> {
    ...
    let status = cargo.status().map_err(|error| {
        Error::Device(format!(
    ...
    if !status.success() {
        return Err(Error::Device(format!(
            "cargo +esp build -p {package} failed; {ESP_TOOLCHAIN_HINT}"
        )));
    }
...
mod tests {
    ...
    #[test]
    fn simple_debug_names() {
        assert_eq!(FirmwareImage::SimpleDebug.package(), "simple-debug-fw");
        assert_eq!(FirmwareImage::SimpleDebug.bin_stem(), "simple-debug");
        assert_eq!(FirmwareImage::EmbassyDebug.package(), "embassy-debug-fw");
        assert_eq!(FirmwareImage::EmbassyDebug.bin_stem(), "embassy-debug");
    }
}
