...
/// Require a dump whose length is the measured full-chip size.
pub fn require_full_dump(dump: &[u8], expected: usize) -> Result<(), Error> {
    if dump.len() != expected {
...
/// On-disk name for a full-chip image (`flash-16mb.bin` when exact MiB).
#[must_use]
pub fn dump_file_name(len: usize) -> String {
    if len > 0 && len.is_multiple_of(1024 * 1024) {
        format!("flash-{}mb.bin", len / (1024 * 1024))
    } else {
        "flash.bin".into()
    }
}
...
mod tests {
    ...
    #[test]
    fn dump_name_for_16mib() {
        assert_eq!(dump_file_name(16 * 1024 * 1024), "flash-16mb.bin");
        assert_eq!(dump_file_name(100), "flash.bin");
    }
}
